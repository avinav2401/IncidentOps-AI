...
Ported from IncidentFox with adaptations for IncidentOps AI:
- Formats AI analysis results as GitHub-flavored markdown
- Supports both PR comments and issue comments
- Token resolution: config → env fallback
"""
...
def _format_markdown(
    ...
    lines.append(f"## \U0001f916 {agent_name} {status_emoji}")
    lines.append("")
    ...
    if not success:
        ...
        if run_id:
            lines.append("")
            lines.append(f"<!-- incidentops:run_id={run_id} -->")
        return "\n".join(lines)
